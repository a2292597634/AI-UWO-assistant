...
每次 `devtools:changed` 运行代表一个可追踪的验收轮次。实现层不要求用户额外填写表单：变更文件由 Git 工作区状态自动收集，场景截图由执行器自动收集；只有需要解释设计取舍时才允许调用方提供摘要或备注。没有上一轮基线时，时间线仍展示“本轮结果”，不虚构 before 截图。
...
`report.html` 是一个可以直接用 Chrome/Edge 打开的自包含文件。页面使用语义化的 `<header>`、`<main>`、`<section>`、`<ol>` 和 `<details>`，窄屏下单列显示，不依赖构建工具。
...
- before 与 after 截图缩略图。只有存在对应路径时才渲染图片；缺失时显示明确的“本轮没有可信基线/截图”。
...
使用 `<details>` 放置格式化后的 `report.json` 片段，便于 AI 排查字段而不影响普通用户阅读。报告不嵌入完整二进制图片，避免 HTML 文件膨胀；图片只以相对路径引用。
...
报告器用 `path.relative(outputDir, screenshotPath)` 生成 HTML `src`，并统一为 `/`。只有解析后位于当前运行目录内的截图才渲染为图片；目录外的路径只作为文本证据显示，防止 HTML 意外暴露任意本机文件。JSON/Markdown 继续保存现有绝对路径语义。
...
- 两种模式都复用已存在的本机 WebSocket 端点；没有连接时才调用 `devtools:start` 逻辑。每个场景完成后释放连接，继续使用现有超时、重试和首帧等待机制，不启动常驻监听器。
...
该规则写入 `AGENTS.md` 和 `docs/miniprogram-review.md`，作为 AI 的强制工作流约束：检测页面变更 → 选择受影响场景 → 调用自动化 → 读取截图/报告 → 修复 → 重跑 → 通过后交付。
...
- 调用层可以提供 `summary` 和 `notes`；缺省值保证不增加普通开发流程的必填步骤。报告器不得自行推断修改原因。
...
打开终端打印的 `report.html`，确认可以看到摘要、场景步骤和截图缩放；连续修改页面并重复命令，确认每次运行的迭代记录包含变更文件和 after 截图。若登录、授权或设备切换阻塞，报告必须清楚显示阻塞原因，并把该项列为人工核验。
...
8. 单元测试和完整 `npm run verify` 通过；真实开发者工具验收结果与报告内容一致。
