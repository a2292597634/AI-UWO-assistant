...
### [x] P1-07：职业筛选项一次性全传，setData 和渲染开销大

- **文件**：`miniprogram/pages/catalog/index.ts`
- **现状**：`onLoad` 时将全部职业字典一次性 `setData`
- **修复**：依产品决定移除目录页职业筛选，不再向目录页传递职业选项、草稿状态或执行职业过滤；航海士资料上的职业字段仍保留给卡片显示及其他流程。
- **状态**：已完成。
- **工作量**：中
...
### [x] P2-04：技能图标无统一错误回退组件

- **状态**：已完成。目录页主动与被动技能已统一使用 skill-icon 组件，组件在图片失败时显示分类首字回退并保留技能名称无障碍语义。
- **修复**：封装 SkillIcon 组件，统一 binderror、分类默认图标、加载占位、无障碍文本
- **工作量**：中

### [x] P2-05：筛选 Chip 无障碍语义不一致

- **文件**：`miniprogram/pages/catalog/index.wxml`
- **现状**：稀有度/类型/性别 Chip 有 `role="button"` 和 `aria-label`，技能/语言/职业没有
- **修复**：保留的稀有度、类型、性别、语言与技能分类筛选均补全 `role="button"`、`aria-label`、`aria-checked`；职业筛选已依产品决定移除。
- **状态**：已完成。
- **工作量**：小
...
```
已完成：P2-02 → P2-03 → P1-08 → P1-02 → P1-03 → P1-04 → P2-01 → P1-07 → P2-05
下一轮：无
```
