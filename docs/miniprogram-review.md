...
5. 如果命令提示环境阻塞，等待自动恢复流程完成；若恢复仍失败，再根据报告中的每次尝试结果处理 CLI、登录、授权或端口问题。
6. 读取生成的 `report.html`，确认报告内已经直接显示修改过程、场景和失败现场截图；`report.json`、`report.md` 只作为结构化兼容输出。
7. 检查信息层级、间距、文字溢出、按钮可发现性、安全区、空状态和交互结果。
8. 修复发现的问题并重复场景，直至自动断言和视觉检查均无阻塞问题。
9. 把 `report.html`、实际覆盖项和待人工项交给用户核验；不得要求用户自行按路径寻找截图。
