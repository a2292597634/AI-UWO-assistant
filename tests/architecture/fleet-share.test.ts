...
describe('配隊分享圖架構契約', () => {
  ...
  it('分享長圖底板使用受控本地素材、透明頁首與繁體中文模式文案', () => {
    ...
    expect(existsSync(resolve(root, 'miniprogram/assets/ui/fleet-share-nautical-motifs.png'))).toBe(
      true,
    )
    expect(source).toContain("'/assets/ui/fleet-share-map.jpg'")
    expect(source).toContain("'/assets/ui/fleet-share-nautical-motifs.png'")
    expect(source).toContain('drawShareBackground')
    expect(source).toContain('drawModeEmblem')
    expect(source).toContain('drawMotifDecorations')
    expect(source).toContain('FLEET_SHARE_MOTIF_SOURCE_RECTS')
    ...
    expect(recipeSource).toContain("id: 'fleet-share-nautical-motifs'")
    expect(recipeSource).toContain("output: 'fleet-share-nautical-motifs.png'")
    expect(recipeSource).toContain('width: 750')
    expect(recipeSource).toContain('height: 1125')
    expect(recipeSource).toContain('maxBytes: 80 * 1024')
  })
})
